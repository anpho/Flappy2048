...
function preloadAudio() {
	PGLowLatencyAudio.preloadAudio("flap.wav", "f2/", 1, function(echoValue) {
		console.log('[OK] flap.wav loaded.');
	});
	PGLowLatencyAudio.preloadAudio("coin.wav", "f2/", 1, function(echoValue) {
		console.log('[OK] coin.wav loaded.');
	});
	PGLowLatencyAudio.preloadAudio("punch.wav", "f2/", 1, function(echoValue) {
		console.log('[OK] punch.wav loaded.');
	});
}
...
var flap = {
	play: function() {
		if (sndo.has_sound) {
			PGLowLatencyAudio.play("flap.wav", function(echoValue) {
...
var coin = {
	play: function() {
		if (sndo.has_sound) {
			PGLowLatencyAudio.play("coin.wav", function(echoValue) {
...
var punch = {
	play: function() {
		if (sndo.has_sound) {
			PGLowLatencyAudio.play("punch.wav", function(echoValue) {
...
logo.showing = true;
logo.hiding = false;
logo.img = loadGameImage('f2/logo.png' /*tpa=http://i.lvme.me/f2/logo.png*/ ); // 626 x 188
...
gameover.showing = false;
gameover.hiding = true;
gameover.img = loadGameImage('f2/gameover.png' /*tpa=http://i.lvme.me/f2/gameover.png*/ ); // 626 x 144
...
fbshare.img.onclick = function() {
	...
	var ss = "file://" + blackberry.io.home + "/highscore.png";
	setTimeout(function() {
	var screenshot = community.screenshot.execute({
		dest: ss,
		mime: 'image/png'
	});
	var title = 'Share';
	...
	blackberry.invoke.card.invokeTargetPicker(request, title, function() {}, function(e) {});
	fbshare.img.style.display = "block";
	playagain.img.style.display = "block";
	},100);
*/
}
...
playagain.showing = false;
playagain.hiding = true;
playagain.img = loadGameImage('f2/playagain.png' /*tpa=http://i.lvme.me/f2/playagain.png*/ );
...
c2s.showing = true;
c2s.hiding = false;
c2s.img = loadGameImage('f2/clicktostart.png' /*tpa=http://i.lvme.me/f2/clicktostart.png*/ ); // 337 x 75
